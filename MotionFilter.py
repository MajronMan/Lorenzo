...
class MotionFilter(ImageFilter):
    def __init__(self, max_background_age=10, min_contour_area=1000, max_contour_area=2000):
    ...
    def filter(self, frame):
        ...
        if self.background_age > self.max_background_age:
            self.background_frame = preprocessed
            self.background_age = 0

        self.background_age += 1

        frame_delta = cv2.absdiff(self.background_frame, preprocessed)
